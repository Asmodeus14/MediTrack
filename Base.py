...
# Print Data
def print_data():
    answer =messagebox.askokcancel("Regarding Print","This Will Print whole Sheet")
    if answer is True:
        conn = sqlite3.connect("patient_app.db")
        cursor = conn.cursor()
        cursor.execute("SELECT * FROM patients")
        ...
        messagebox.showwarning("Regarding Print","Process was intreupted")
...
def Specific_data_print(tree):
    ...
    # Create a new PDF in memory with the added text
    packet = io.BytesIO()
    ...
    # Merge the new PDF with the specified page of the original PDF
    for i, page in enumerate(reader.pages):
        if i == 0:
            page.merge_page(new_pdf.pages[0])  # Overlay the new PDF content
        writer.add_page(page)

    # Write the final output to a new PDF file
...
def search_patients(search_entry, tree):
    ...
    if state is True:
        ...
        messagebox.showerror("Search","Paitent not found \n Try finding manually if paitent knows Date and Id")
...
def main():
    ...
    # Start the database backup thread
    ...
    root.title("Patient Info and Credentials App")
    root.state("zoomed")
    ...
    tree_patients.heading("Adress", text="Adress",anchor="center")
    ...
    tree_patients.column("Adress",anchor="w")
    ...
    ttk.Button(frame_buttons, text="Print All", command=print_data).pack(side="left", padx=5)
    ttk.Button(frame_buttons, text="Print", command=lambda :Specific_data_print(tree_patients)).pack(side="left", padx=5)
